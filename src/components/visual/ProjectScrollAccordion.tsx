...
export function ProjectScrollAccordion({ items }: { items: Project[] }) {
  ...
  const scrollToIndex = (index: number) => {
    ...
    const targetScroll =
      absoluteTop + progress * (rect.height - window.innerHeight);
    ...
    >
      ...
      <div className="sticky top-0 h-screen w-full hidden md:flex flex-col justify-center overflow-hidden">
        <div className="container-padded mb-8">
          ...
          <p className="mt-3 text-cream-800">
          ...
          {items.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative h-full overflow-hidden rounded-[24px] border-2 border-brand-700 flex-shrink-0 cursor-pointer"
              ...
              >
                <Image
                  ...
                  className="object-cover z-10 transition-transform duration-1000 ease-out group-hover:scale-110"
                />

                <div className="absolute inset-0 bg-brand-800 z-5 pointer-events-none" />
                ...
                <AnimatePresence>
                  {isActive && (
                    ...
                    >
                      <span className="mb-4 inline-flex w-fit rounded-full border-2 border-sunrise-500 bg-sunrise-400 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-brand-950">
                        {item.tag}
                      </span>
                      <h3 className="font-bold text-cream text-3xl mb-3 min-w-[50vw] w-[85%]">
                        {item.title}
                      </h3>
                      <p className="text-base md:text-lg leading-relaxed text-cream bg-brand-900 p-5 rounded-2xl border-2 border-brand-700 min-w-[51vw] w-[85%]">
                        {item.description}
                      </p>
                    </motion.div>
                  )}
              ...
              >
                ...
                <AnimatePresence>
                  {isActive && (
                    ...
                    >
                      <span className="mb-2 inline-flex w-fit rounded-full border-2 border-sunrise-500 bg-sunrise-400 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-950">
                        {item.tag}
                      </span>
                ...
                <AnimatePresence>
                  {!isActive && (
                    <motion.div
                      ...
                      transition={{ duration: 1.0 }}
                    >
                      <h3 className="font-semibold text-cream tracking-wider text-sm truncate max-w-[250px]">
                        {item.title}
                      </h3>
                    </motion.div>
                  )}
